...
Finally, we pass calibration-derived sequence rules through an AIRIS/DAS-shaped retrieval service and isolate forecast transport integrity. Across 6,670 paired clean and fault-injection trials, digest sealing preserves 100\% clean acceptance while reducing negative-control acceptance from 44.4\% under topology-only arbitration to zero. This is a deterministic authorization result, not evidence for learned AIRIS robustness.
Independently induced context rules achieve 94.0\% held-out episode-winner precision, but exactly reproduce the
frozen controller; confidence demotion therefore changes acceptance without changing behavior. Storyworld
precision falls to 65.6\%--78.1\%, motivating state-conditioned typed fallback rather than context confidence alone.
\end{abstract}
...
\begin{figure}[t]
\centering
\input{figures/control_dynamics.tex}
...
\subsection{Topology-Controlled Skill Sequencing}

We next isolate the effect of the control policy while holding the skill implementations fixed. Every context
exposes the same proposal-only TRM, deduction-only LDT, and typed propose/certify hybrid sequences. Sequence
scores are fitted on calibration data only. Two calibration folds define transport estimates
$T_c^{(1)},T_c^{(2)}$ over the preference vector
$(u_{\mathrm{typed}}-u_{\mathrm{TRM}},u_{\mathrm{typed}}-u_{\mathrm{LDT}})$. Their lineage contraction
$\lambda_c$, loop disagreement $h_c$, and orientation flag $o_c$ produce the conservative authorization rule
\[
  \widehat R_c=\min\{2,\lambda_c+h_c\},\qquad
  \textsc{global}(c)\iff o_c=0\ \land\ \widehat R_c\leq\epsilon
\]
after simultaneous-coverage and matched-noise checks. An orientation reversal forces $\widehat R_c=2$.
Unauthorized contexts use the best calibration-fitted local section. Figure~\ref{fig:sequencer-control} shows
the resulting flow.

\begin{figure}[t]
\centering
\resizebox{\textwidth}{!}{\input{figures/sequencer_control.tex}}
\caption{Matched topology-controlled sequencer experiment. Training/calibration, transport receipts, policy
authorization, and paired evaluation remain separate.}
\label{fig:sequencer-control}
\end{figure}

The frozen evaluation contains 667 paired episodes: 96 Sudoku automorphisms, 168 procedural ARC-1 instances,
160 procedural ARC-2 instances, 115 held-out local Tesseract routing trajectories, and 128 storyworld starts
split equally between secret-ending and moral objectives. Families receive equal macro weight. We use 5,000
paired family-stratified hierarchical context-cluster bootstrap resamples, 10,000 context-cluster sign flips,
Holm correction over three registered controls, family-balanced cluster Cohen's $d_z$, and an exact McNemar
accuracy audit. ARC instances are retained only when all candidate sequences solve them, isolating sequencer
efficiency rather than solver coverage.
...
Control math & \textbf{0.9001} & \textbf{0.9667} & \textbf{6.436} & \textbf{0.0000} \\
\bottomrule
\end{tabular}
\caption{Registered matched-sequencer result. Control math changes selection, not the candidate skills.}
\label{tab:sequencer-control}
\end{table}

Against global signed control, the paired macro-utility difference is $+0.006943$ with clustered 95\% interval
$[+0.004909,+0.009232]$, Holm-adjusted $p=0.000600$, and family-balanced cluster $d_z=0.876$. Accuracy does not
change (exact McNemar $p=1$), while macro cost decreases by 1.238. Against lineage-only control, the difference
is only $+0.000310$ ($[0,+0.000864]$, clustered $p=0.502350$, $d_z=0.209$), with cost lower by 0.017. Thus this
slice does not establish incremental utility from holonomy/orientation checks beyond lineage localization. The
registered controller sections 100\% of contexts and equals the local-calibrated reference. The positive result
is conservative localization, not broad evidence for safe global signed transport. Orientation reversal is
found in 14.3\% of contexts.

\begin{table}[t]
\centering
\begin{tabular}{rrrr}
\toprule
$\epsilon$ & Local-section rate & Macro utility & Macro cost \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Post-registration sensitivity. Relaxing signed-control authorization reduces localization but erodes
utility and raises cost on this benchmark slice.}
\label{tab:sequencer-sensitivity}
\end{table}

For portable replay, the 667 immutable tasks are also packaged under the Verifiers 0.1.14 v1
Taskset/Harness API~\cite{primeintellect2026verifiers}. The Taskset owns tasks and scoring; the Harness owns the
sequencer program. This is an LLM-free harness integration artifact, not an additional model result.

\subsection{AIRIS/DAS Forecast Integrity}

We next pass the calibration-derived context plans through an AIRIS-compatible symbolic rule contract and a
DAS-shaped retrieval service. AIRIS emphasizes interpretable causal rules and a dynamic internal world
model~\cite{airis2026}; Hyperon provides the adjacent MeTTa-oriented symbolic execution context~\cite{hyperon2026}.
Our local service is an in-memory DAS-shaped index, not a native distributed DAS deployment. AIRIS/DAS proposes
a sequence, while the topology membrane retains execution authority.

For retrieved rule $r$, define canonical decision material and an integrity predicate
\[
  m(r)=(\mathrm{id},\mathrm{confidence},\mathrm{support},\mathrm{counterexample\ count},
  \mathrm{preconditions},\mathrm{prediction}),
  \qquad
  I(r)=\mathbf 1\!\left[\operatorname{SHA256}(m(r))=D_{\mathrm{id}(r)}\right],
\]
where $D$ is frozen from calibration-exported rules. If $G(c,r)$ denotes the existing exact-match, protocol,
context, support, route, and orientation gates, the executed sequence is
\[
  q(c)=
  \begin{cases}
  ...
  \end{cases}
\]
where $q_A$ is the retrieved AIRIS proposal and $q_T$ is the frozen topology-control fallback.
Figure~\ref{fig:airis-integrity} shows the authority split.

\begin{figure}[t]
\centering
\resizebox{\textwidth}{!}{\input{figures/airis_das_integrity.tex}}
\caption{AIRIS/DAS retrieval with a frozen rule digest. Semantic topology checks and transport integrity are
independent gates; either failure returns control to the frozen local sequencer.}
\label{fig:airis-integrity}
\end{figure}

We apply ten paired conditions to each of the 667 evaluation episodes: one clean positive control and nine
protocol, unseen-context, sequence-substitution, confidence/support inflation, route/authority override,
unknown-rule, and outage negative controls. Candidate outcomes remain fixed. The topology-only ablation omits
$I(r)$ but retains every semantic gate.
...
\bottomrule
\end{tabular}
\caption{Paired AIRIS/DAS receipt fault injection over 6,670 trials. Regret is measured against the frozen
\texttt{control\_math} selection.}
\label{tab:airis-integrity}
\end{table}

Topology checks already reject protocol shifts, unseen contexts, route and authority overrides, and outages.
Without the digest, they accept every sequence substitution, confidence inflation, support inflation, and
unknown rule identifier, producing a 44.4\% aggregate fault-acceptance rate. Integrity sealing rejects all 6,003
negative controls while accepting all 667 clean controls. This isolates transport integrity. It does not show
that an intact learned rule is correct, nor does it evaluate adversarial robustness of an AIRIS learner. The
registry itself requires an external trust anchor; the live client checks the ruleset against the independently
saved bridge-result SHA-256 before constructing the registry.

\subsection{Calibration-Outcome Rule Induction}

Transport integrity does not answer whether an intact rule is useful. We therefore freeze a second protocol
that induces one empirical AIRIS-style rule per context from 455 calibration episodes, without reading held-out
outcomes or copying the topology controller's sequence label. For calibration episodes $\mathcal C_c$ in context
$c$, let
\[
  y_e=\underset{q\in\mathcal Q}{\arg\max}\;u_e(q),\qquad
  q_A(c)=\underset{q\in\mathcal Q}{\arg\max}\sum_{e\in\mathcal C_c}\mathbf 1[y_e=q],
\]
with a preregistered tie order. The rule's support, confidence, and counterexample set are
\[
  s_c=\sum_{e\in\mathcal C_c}\mathbf 1[y_e=q_A(c)],\qquad
  \gamma_c=\frac{s_c}{|\mathcal C_c|},\qquad
  K_c=\{e\in\mathcal C_c:y_e\ne q_A(c)\}.
\]
Topology contributes only protocol, route, orientation, and risk features used by the authorization membrane.
The selected sequence and confidence are learned from calibration utility winners. This is empirical
context-rule induction, not causal identification or neural AIRIS training.

The 28 induced rules are evaluated on the same 667 held-out episodes. Raw held-out winner-label accuracy is
94.0\%, but the errors are concentrated in heterogeneous contexts: all Sudoku and ARC labels are correct,
routing has four errors in 115 episodes, secret-ending play has 22 in 64, and moral-optimization play has 14 in
64 (Table~\ref{tab:airis-induction}).
...
\bottomrule
\end{tabular}
\caption{Held-out episode-winner precision of calibration-induced context rules. Story contexts contain
state-dependent tradeoffs that a single context-level confidence cannot represent.}
\label{tab:airis-induction}
\end{table}

This result is deliberately negative at the control level. Every induced proposal equals the frozen
\texttt{control\_math} context choice, so raw proposal/control parity is 1.000, macro utility is 0.9001 for both,
and guarded utility delta is zero. Raising the confidence threshold from 0.5 to 1.0 lowers acceptance from 1.000
to 0.760 and lowers intact-wrong-rule acceptance from 1.000 to zero, but rejected rules fall back to the same
context choice. Consequently utility, accuracy, and cost do not change. Confidence here identifies uncertain
contexts but cannot produce a better action without a behaviorally distinct fallback. A state-conditioned rule
or typed reachability certificate is required to split secret-ending and moral-optimization episodes within the
same storyworld context.
...
\begin{figure}[t]
\centering
\resizebox{\linewidth}{!}{\input{figures/hrm_training_review.tex}}
...
The benchmarks are small and synthetic. They test contracts, not scale. The TRM implementations in this repository are dependency-light analogues, not full recurrent neural models~\cite{wang2025hrm,jolicoeurmartineau2025trm}. The Tesseract router reference uses a TF-IDF plus MLP architecture, and the local routing benchmark mirrors the classification objective without importing its full PyTorch stack~\cite{tesseract2026router}. The LDT side is symbolic and domain-specific rather than a learned lattice transformer~\cite{davis2026ldt}. A learned LDT head is future work. The hybrid has no aggregate score regression in the saved benchmarks, but it does have efficiency regressions in ARC-2 subcases and no routing accuracy gain over TRM yet. The sequencer study replays deterministic known-task skills; its significant gain over global control is mainly cost and preference selection, not higher accuracy, and it shows no significant advantage over lineage-only control. ARC tasks are filtered for common solver success. The AIRIS/DAS integrity study injects specified deterministic receipt faults and does not exercise native distributed DAS. The induction study learns empirical context rules, not causal rules or a neural AIRIS model; its fallback is behaviorally identical to its proposals, so confidence demotion cannot demonstrate utility protection. The rule registry requires a trust anchor outside the mutable rule store. These results cannot be interpreted as neural weight infusion or official ARC/INTELLECT-3 performance. The HRM training-review matrix uses synthetic sealed receipts; it validates fail-closed routing but supplies no evidence about neural training gains. The signed-control theorem is limited to registered coordinate error under its coverage assumptions and cannot establish general behavioral safety.
...
\begin{enumerate}
  ...
  \item Replace deterministic sequencer candidates with frozen learned TRM/LDT checkpoints while retaining the
  registered partitioning and paired control-policy ablations.
  \item Add state-conditioned AIRIS features for story reachability and routing observations, then compare a
  behaviorally distinct typed fallback against confidence-only demotion under frozen calibration partitions.
