...
public static class LsaKappaSurfaceJobObject {
...
$job = [LsaKappaSurfaceJobObject]::CreateJobObject([IntPtr]::Zero, "lsa-kappa-surface-$PID-$Phase")
$limit = New-Object LsaKappaSurfaceJobObject+ExtendedInfo
$limit.basic.flags = [LsaKappaSurfaceJobObject]::ProcessMemory -bor [LsaKappaSurfaceJobObject]::KillOnClose
...
if (-not [LsaKappaSurfaceJobObject]::SetInformationJobObject($job, [LsaKappaSurfaceJobObject]::Extended, $ptr, $size)) {
    throw "memory cap setup failed"
}
[Runtime.InteropServices.Marshal]::FreeHGlobal($ptr)
$cpu = New-Object LsaKappaSurfaceJobObject+CpuInfo
$cpu.flags = [LsaKappaSurfaceJobObject]::CpuEnable -bor [LsaKappaSurfaceJobObject]::CpuHardCap
$cpu.rate = $CpuRate
$size = [Runtime.InteropServices.Marshal]::SizeOf($cpu)
$ptr = [Runtime.InteropServices.Marshal]::AllocHGlobal($size)
[Runtime.InteropServices.Marshal]::StructureToPtr($cpu, $ptr, $false)
if (-not [LsaKappaSurfaceJobObject]::SetInformationJobObject($job, [LsaKappaSurfaceJobObject]::Cpu, $ptr, $size)) {
...
try {
    ...
    [LsaKappaSurfaceJobObject]::CloseHandle($job) | Out-Null
    $finished = Get-Date
