param(
    ...
    [ValidateSet("materialize", "calibrate", "validate", "run", "finalize")]
    [string]$Phase,
    [string]$Config = "configs/rlm_uncertainty_pregate_v0_1.json",
    [string]$Output = "experiments/rlm_uncertainty_pregate_v0_1",
    [string]$Module = "research_gym.scripts.bench_rlm_uncertainty_pregate_v0_1",
...
public static class RlmUncertaintyPregateJobObject {
...
$job = [RlmUncertaintyPregateJobObject]::CreateJobObject([IntPtr]::Zero, "rlm-uncertainty-pregate-$PID-$Phase")
$limit = New-Object RlmUncertaintyPregateJobObject+ExtendedInfo
$limit.basic.flags = [RlmUncertaintyPregateJobObject]::ProcessMemory -bor [RlmUncertaintyPregateJobObject]::KillOnClose
...
if (-not [RlmUncertaintyPregateJobObject]::SetInformationJobObject($job, [RlmUncertaintyPregateJobObject]::Extended, $ptr, $size)) {
    throw "memory cap setup failed"
}
[Runtime.InteropServices.Marshal]::FreeHGlobal($ptr)
$cpu = New-Object RlmUncertaintyPregateJobObject+CpuInfo
$cpu.flags = [RlmUncertaintyPregateJobObject]::CpuEnable -bor [RlmUncertaintyPregateJobObject]::CpuHardCap
$cpu.rate = $CpuRate
$size = [Runtime.InteropServices.Marshal]::SizeOf($cpu)
$ptr = [Runtime.InteropServices.Marshal]::AllocHGlobal($size)
[Runtime.InteropServices.Marshal]::StructureToPtr($cpu, $ptr, $false)
if (-not [RlmUncertaintyPregateJobObject]::SetInformationJobObject($job, [RlmUncertaintyPregateJobObject]::Cpu, $ptr, $size)) {
...
try {
    ...
    if (-not [RlmUncertaintyPregateJobObject]::AssignProcessToJobObject($job, $proc.Handle)) {
    ...
    [RlmUncertaintyPregateJobObject]::CloseHandle($job) | Out-Null
    $finished = Get-Date
