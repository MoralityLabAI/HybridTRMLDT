param(
    ...
    [ValidateSet("validate", "fit_r32", "holdout_r64", "finalize")]
    [string]$Phase,
    [string]$Config = "configs/loop_schedule_algebra_saturation_addendum_v1.json",
    [string]$Output = "experiments/loop_schedule_algebra_saturation_addendum_v1",
...
$PriorReceipts = @(
    ...
    Where-Object { $_.Name -match "^(fit_r32|holdout_r64)\.attempt-" }
)
...
public static class LsaSaturationJobObject {
...
$job = [LsaSaturationJobObject]::CreateJobObject([IntPtr]::Zero, "lsa-saturation-$PID-$Phase")
$limit = New-Object LsaSaturationJobObject+ExtendedInfo
$limit.basic.flags = [LsaSaturationJobObject]::ProcessMemory -bor [LsaSaturationJobObject]::KillOnClose
...
if (-not [LsaSaturationJobObject]::SetInformationJobObject($job, [LsaSaturationJobObject]::Extended, $ptr, $size)) {
    throw "memory cap setup failed"
}
[Runtime.InteropServices.Marshal]::FreeHGlobal($ptr)
$cpu = New-Object LsaSaturationJobObject+CpuInfo
$cpu.flags = [LsaSaturationJobObject]::CpuEnable -bor [LsaSaturationJobObject]::CpuHardCap
$cpu.rate = $CpuRate
$size = [Runtime.InteropServices.Marshal]::SizeOf($cpu)
$ptr = [Runtime.InteropServices.Marshal]::AllocHGlobal($size)
[Runtime.InteropServices.Marshal]::StructureToPtr($cpu, $ptr, $false)
if (-not [LsaSaturationJobObject]::SetInformationJobObject($job, [LsaSaturationJobObject]::Cpu, $ptr, $size)) {
    throw "CPU cap setup failed"
}
[Runtime.InteropServices.Marshal]::FreeHGlobal($ptr)

$arguments = @(
    "-m", "research_gym.scripts.bench_loop_schedule_algebra_saturation_addendum",
    "--phase", $Phase,
    "--config", $ConfigPath,
    "--output", $OutputPath,
...
try {
    ...
    if (-not [LsaSaturationJobObject]::AssignProcessToJobObject($job, $proc.Handle)) {
    ...
    [LsaSaturationJobObject]::CloseHandle($job) | Out-Null
    $finished = Get-Date
    $memoryAfter = Get-MemoryAudit
    $ownedGpuAfter = @($memoryAfter.gpu_compute_apps | Where-Object { $_ -match "^\s*$($proc.Id)\s*," })
